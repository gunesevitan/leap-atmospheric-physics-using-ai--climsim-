...
if __name__ == '__main__':
    ...
    for seed in seeds:
        ...
        validation_dataset = xgb.DMatrix(df_train.loc[validation_idx, features], label=df_train.loc[validation_idx, target])
        ...
        df_feature_importance_gain['importance'] += (pd.Series(model.get_score(importance_type='gain')).fillna(0) / len(seeds))
        df_feature_importance_weight['importance'] += (pd.Series(model.get_score(importance_type='weight')).fillna(0) / len(seeds))
        df_feature_importance_cover['importance'] += (pd.Series(model.get_score(importance_type='cover')).fillna(0) / len(seeds))

        df_train.loc[validation_mask, 'prediction'] += (model.predict(xgb.DMatrix(df_train.loc[validation_idx, features])) / len(seeds))
        df_test.loc[:, 'prediction'] += (model.predict(xgb.DMatrix(df_test.loc[:, features])) / len(seeds))
